...
import 'internal_category_form_screen.dart';
import 'ticket_models.dart';

class InternalCategoriesScreen extends StatefulWidget {
  const InternalCategoriesScreen({
    super.key,
  ...
  @override
  State<InternalCategoriesScreen> createState() =>
      _InternalCategoriesScreenState();
}

class _InternalCategoriesScreenState extends State<InternalCategoriesScreen> {
  final _searchController = TextEditingController();
  final _scrollController = ScrollController();
  final _loader = AppAsyncListLoader<List<InternalCategory>>();
  String _searchQuery = '';
  ...
  Future<List<InternalCategory>> _loadData() {
    return widget.apiClient.getInternalCategories();
  }
  ...
  int _compareCategories(InternalCategory a, InternalCategory b) {
  ...
  Future<void> _addCategory() async {
    final saved = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => InternalCategoryFormScreen(
  ...
  Future<void> _editCategory(InternalCategory category) async {
    final saved = await Navigator.of(context).push<bool>(
      MaterialPageRoute(
        builder: (_) => InternalCategoryFormScreen(
          apiClient: widget.apiClient,
  ...
  Widget build(BuildContext context) {
    return Theme(
      ...
      child: AppScreenScaffold(
        ...
        body: SafeArea(
          child: FutureBuilder<List<InternalCategory>>(
            ...
            builder: (context, snapshot) {
              ...
              if (snapshot.hasError) {
                return AppLoadErrorState(
                  title: 'Failed to load internal categories',
              ...
              final categories = snapshot.data ?? const <InternalCategory>[];
                      ...
                      children: [
                        ...
                        Expanded(
                          child: filtered.isEmpty
                              ? const Center(
                                  child: Text(
                                    'No internal categories match the search.',
...
class _InternalCategoryCard extends StatelessWidget {
  const _InternalCategoryCard({
